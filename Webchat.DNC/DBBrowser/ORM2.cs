...
namespace ArachnidCreations.SqliteDAL
{
    public class ORM
    {
        ...
        public static bool OverrideDateFormat = true;
        ...
        {
            return GetSingle<T>(tablename, id.ToString(), keyname);
        }
                    ...
                    foreach (PropertyInfo prop in props)
                    {
                        ...
                        if (prop.PropertyType.ToString().ToLower().Contains("datetime"))
                        {
                            if (OverrideDateFormat)
                            {
                                ...
                                var dta = DateTime.ParseExact(row[prop.Name].ToString(), "dddd, MMMM dd, yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture);

                                // create a string representing the DateTime in another format
                                string output = dta.ToString("M/dd/yyyy hh:mm:ss", System.Globalization.CultureInfo.InvariantCulture);
